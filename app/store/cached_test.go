...
func TestCached_SetWithVersion(t *testing.T) {
	t.Run("invalidates cache on successful update", func(t *testing.T) {
		...
		info, err := cached.GetInfo("key1")
		require.NoError(t, err)

		// update with matching version
		err = cached.SetWithVersion("key1", []byte("updated"), "json", info.UpdatedAt)
		require.NoError(t, err)
	...
	t.Run("returns conflict error on version mismatch", func(t *testing.T) {
		...
		// get initial version
		info1, err := cached.GetInfo("key1")
		require.NoError(t, err)

		// simulate concurrent update
		time.Sleep(1100 * time.Millisecond)
		require.NoError(t, cached.Set("key1", []byte("concurrent"), "yaml"))

		// try to update with old version
		err = cached.SetWithVersion("key1", []byte("my-update"), "json", info1.UpdatedAt)
	...
	t.Run("zero time behaves like regular set", func(t *testing.T) {
		...
		err = cached.SetWithVersion("key1", []byte("value1"), "text", time.Time{})
		require.NoError(t, err)
